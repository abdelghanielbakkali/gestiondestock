...
export default function Header() {
    ...
    const fetchUnread = async () => {
        try {
            // Pour admin, on compte toutes les notifications non lues (pas de filtre par type)
    ...
    const getPhotoUrl = () => {
        ...
        if (user?.photo) {
            return `${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}/storage/${user.photo}`;
        }
        return null;
    };
    ...
    return (
        <>
            ...
            {/* Carte de profil simple et moderne */}
            {showProfileModal && (
